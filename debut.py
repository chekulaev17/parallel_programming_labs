...
def init_csv():
    with open(RESULTS_FILE, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Matrix_Size', 'Threads', 'Time_ms', 'GFLOPS', 'Verified'])

def save_result(n, threads, metrics, verified):
    with open(RESULTS_FILE, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([n, threads, f"{metrics['time_ms']:.4f}", f"{metrics['gflops']:.2f}", 'YES' if verified else 'NO'])

def main():
    print("=== Тестирование OpenMP ===\n")
    ...
    for n in MATRIX_SIZES:
        ...
        for threads in THREAD_COUNTS:
            print(f"  Потоков: {threads}...", end=" ", flush=True)
