...
EXEC = "matmul_cuda.exe"
MATRIX_SIZES = [200, 400, 800, 1200, 1600, 2000]
BLOCK_CONFIGS = [8, 16, 32]
...
def init_csv():
    with open(RESULTS_FILE, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Matrix_Size', 'Block_Size', 'Time_ms', 'GFLOPS', 'Verified'])

def save_result(n, block, metrics, verified):
    with open(RESULTS_FILE, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([n, block, f"{metrics['time_ms']:.4f}", f"{metrics['gflops']:.2f}", 'YES' if verified else 'NO'])

def main():
    print("=== Тестирование CUDA ===\n")
    ...
    for n in MATRIX_SIZES:
        ...
        for block in BLOCK_CONFIGS:
            print(f"  Блок: {block}x{block}...", end=" ", flush=True)
