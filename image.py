...
df['Volume_GFLOPs'] = (2 * df['Matrix_Size']**3 - df['Matrix_Size']**2) / 1e9

PROC_STYLES = {
    1: {'color': 'blue', 'marker': 'o', 'label': '1 proc'},
    2: {'color': 'green', 'marker': 's', 'label': '2 procs'},
    4: {'color': 'red', 'marker': '^', 'label': '4 procs'},
    6: {'color': 'purple', 'marker': 'd', 'label': '6 procs'},
    8: {'color': 'orange', 'marker': 'v', 'label': '8 procs'},
    12: {'color': 'brown', 'marker': '*', 'label': '12 procs'},
}
...
for procs in sorted(df['Processes'].unique()):
    ...
    plt.plot(subset['Matrix_Size'], subset['Time_ms'], marker=style['marker'], color=style['color'], label=style['label'], linewidth=2, markersize=6)
...
for procs in sorted(df['Processes'].unique()):
    subset = df[df['Processes'] == procs]
    style = PROC_STYLES.get(procs, {'color': 'gray', 'marker': 'x', 'label': f'{procs} procs'})
    plt.plot(subset['Volume_GFLOPs'], subset['Time_ms'], marker=style['marker'], color=style['color'], label=style['label'], linewidth=2, markersize=6)
